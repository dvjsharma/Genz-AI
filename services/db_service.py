...
def connect_to_database() -> Database:
    ...
    try:
        ...
        if not endpoint or not token:
            raise ValueError("Both ASTRA_DB_API_ENDPOINT and ASTRA_DB_APPLICATION_TOKEN environment variables must be set.")
...
def create_or_get_collection(database: Database, collection_name: str):
    ...
    try:
        ...
        try:
            ...
            logging.info(f"Created collection '{collection.full_name}' with COSINE metric.")
        except Exception as create_error:
            logging.error(f"Failed to create collection '{collection_name}': {create_error}")
            raise RuntimeError(f"Failed to create collection '{collection_name}': {create_error}")
    return collection

def upload_csv_to_vector_collection(collection, csv_data: str, vectorize_column: str, chunk_size: int = 50):
    """
    ...
    try:
        ...
        for i in range(0, len(documents), chunk_size):
            ...
            try:
                ...
                logging.info(f"Inserted {chunk_inserted} items in chunk {i // chunk_size + 1}.")
            except Exception as e:
                logging.error(f"Error inserting chunk {i // chunk_size + 1}: {e}")
        
        logging.info(f"Successfully inserted {total_inserted} items into the collection.")
        ...
        raise RuntimeError(f"An unexpected error occurred during the CSV upload process: {e}")